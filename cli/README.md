...
ticket-to-ride/
  ...
  embeddings.bin   ← card embeddings (generated by training pipeline)
  labels.txt       ← card label list (generated by training pipeline)
```
...
The playground UI is also available on the same port at `http://localhost:3000/playground/`. It is pre-loaded with the current game and locked to that game only, and lets you drag cards onto a virtual table to test your scorer in the browser without the app.
