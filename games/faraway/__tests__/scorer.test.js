...
function ctx(players) {
  return { players, similarityThreshold: 0.85 };
}

// ---------------------------------------------------------------------------
describe('processCards – basic contract', () => {
  ...
  it('scores 0 for a player with no cards', () => {
    const results = processCards([], ctx(['Alice']));
...
// ---------------------------------------------------------------------------
describe('processCards – fixed-point regions', () => {
  it('awards fixed points for region:03 (fixed 4)', () => {
    const results = processCards([card('region:03')], ctx(['Alice']));
  ...
  it('sums fixed points for two regions (region:03 + region:09 = 9)', () => {
    ...
    const results = processCards([card('region:03', 0.1), card('region:09', 0.4)], ctx(['Alice']));
...
describe('processCards – perHint tasks', () => {
  ...
  it('scores perHint ×2 for region:11 using hints from revealed region:22', () => {
    ...
    const results = processCards(
      [card('region:11', 0.1), card('region:22', 0.4)],
      ctx(['Alice']),
    );
...
// ---------------------------------------------------------------------------
describe('processCards – condition handling', () => {
  it('returns 0 and "Bedingung nicht erfüllt" when condition is not met', () => {
    ...
    const results = processCards([card('region:17')], ctx(['Alice']));
    expect(results[0].totalScore).toBe(0);
    expect(results[0].cardDetails[0]).toMatchObject({
      cardId: 'region:17',
      points: 0,
...
// ---------------------------------------------------------------------------
describe('processCards – sanctuaries', () => {
  it('scores sanctuary:01 (perYellowOrBlue ×1) against all cards', () => {
    ...
    const results = processCards(
      [card('region:09', 0.1), card('sanctuary:01', 0.4)],
      ctx(['Alice']),
    );
  ...
  it('includes sanctuary landscape in sanctuary scoring scope', () => {
    ...
    const results = processCards(
      [card('sanctuary:01', 0.1), card('sanctuary:02', 0.4)],
      ctx(['Alice']),
    );
...
// ---------------------------------------------------------------------------
describe('processCards – card ID formatting', () => {
  ...
  it('returns "keine Aufgabe" for a region with no scoring task', () => {
    ...
    const results = processCards([card('region:01')], ctx(['Alice']));
    expect(results[0].cardDetails[0].reason).toBe('keine Aufgabe');
  });
});

// ---------------------------------------------------------------------------
describe('processCards – multiple players', () => {
